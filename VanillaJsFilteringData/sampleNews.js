...
let count = 0;
function charactersCount(_object) {
...
spacesCount(sampleNews);


// Mətndəki cümlə sayını return edən funksiya yazın

let count3 = 0;
function sentencesCount(_object) {
  for (let i = 0; i < _object.length; i++) {
    if (_object[i] == "." || _object[i] == "?"  || _object[i] == "!") {
      count3++;
    }
  }
